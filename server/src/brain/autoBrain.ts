...
export class AutoBrainService {
    ...
    private async triggerSdkReview(
        ...
        const reviewPrompt = `对话汇总同步：主 session 已完成第 ${roundNumbers} 轮对话（AI 已结束回复，当前空闲）。请直接调用 brain_summarize 获取对话汇总，然后审查代码改动。审查完成后必须二选一：有问题则 brain_send_message 发送意见，没问题则输出 [NO_MESSAGE]。`
