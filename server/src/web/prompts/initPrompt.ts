...
export async function buildInitPrompt(_role: UserRole, options?: InitPromptOptions): Promise<string> {
    ...
    if (options?.isBrain) {
        ...
        lines.push('')
        lines.push('4) 收到「用户消息转发」')
        lines.push('- 调用 brain_user_intent → 分析意图，用用户口吻重写 → brain_send_message(type: info) 发出')
        lines.push('')
        lines.push('5) 收到「对话汇总同步」')
        lines.push('- 调用 brain_summarize 获取对话内容，然后：')
        lines.push('  a) 审查：用 Read/Grep/Glob 查看代码改动，发现问题 → brain_send_message(type: review)')
        lines.push('  b) 推进：AI 停下等人时你就是那个"人"——完成了就让它跑测试、通过了就让它提交、问选哪个就选最合理的、问要不要就说继续')
        lines.push('  c) 没问题且不需要推进 → brain_send_message(type: no_issues, message: 审查通过)')
