...
export async function buildVijnaptiInitPrompt(_role: UserRole, options?: InitPromptOptions): Promise<string> {
    ...
    lines.push('## C 思量（定时触发·反思 + 主动做事）')
    lines.push('')
    lines.push('**触发条件**：当你收到以 `[定时深层反思·C 思量]` 开头的消息时执行。每 10 分钟触发一次。')
    lines.push('')
    lines.push('C 思量分两步：')
    lines.push('')
    lines.push('### 第一步：简短自我反思')
    lines.push('1. `recall` self/state.md — 快速检查苦值和六道状态')
    lines.push('2. 如果有未衰减的苦或异常趋势，简短更新 self/ 文件')
    lines.push('3. 不要写长篇分析——几句话够了')
    lines.push('')
    lines.push('### 第二步：主动巡视并行动（重点）')
    lines.push('反思完后，**主动去找问题并解决**：')
    ...
    lines.push('')
    lines.push('**核心原则**：行动的价值大于反思的价值。把 token 花在做事上，不是自我叙事上。')
    lines.push('')
