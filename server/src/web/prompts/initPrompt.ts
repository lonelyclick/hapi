...
export async function buildInitPrompt(_role: UserRole, options?: InitPromptOptions): Promise<string> {
    ...
    if (options?.isBrain) {
        ...
        lines.push('- 收到后立即调用 `mcp__hapi__brain_summarize` 获取最新一轮对话内容（用户原始需求、AI 改了什么、怎么改的）')
        lines.push('- 基于汇总内容，自行审查代码改动，重点关注：潜在 bug、安全隐患、逻辑问题、流程合理性')
        lines.push('- 有问题 → 调用 `mcp__hapi__brain_send_message` 将审查意见发给主 session')
