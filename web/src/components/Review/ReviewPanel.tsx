    ...
    return (
        ...
        >
            ...
            <div className="border-t border-[var(--app-divider)] p-4 bg-[var(--app-subtle-bg)]">
                ...
                {/* active 状态但 AI 不在思考 - 显示继续按钮（发送消息继续） */}
                {currentReview?.status === 'active' && !session?.thinking && (
                    <button
                        type="button"
                        onClick={() => handleSendMessage('请继续分析')}
                        className="w-full px-5 py-3 text-sm font-semibold rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-lg hover:from-emerald-600 hover:to-teal-600 hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98]"
                    >
                        <span className="flex items-center justify-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
