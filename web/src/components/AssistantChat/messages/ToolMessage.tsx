        ...
        <div className="flex flex-col gap-3">
            {props.blocks.map((block) => {
                ...
                if (block.kind === 'agent-event') {
                    ...
                    const isAdvisorSuggestion = block.event.type === 'advisor-suggestion'
                    const isAdvisorStatus = block.event.type === 'advisor-suggestion-status'

                    // Special styling for Advisor suggestions
                    if (isAdvisorSuggestion) {
                        const e = block.event as Record<string, unknown>
                        const severity = e.severity as string
                        const detail = e.detail as string || ''
                        const borderColor = severity === 'critical' ? 'border-red-500/50'
                            : severity === 'high' ? 'border-amber-500/50'
                            : severity === 'medium' ? 'border-purple-500/50'
                            : 'border-blue-500/50'
                        const bgColor = severity === 'critical' ? 'bg-red-500/10'
                            : severity === 'high' ? 'bg-amber-500/10'
                            : severity === 'medium' ? 'bg-purple-500/10'
                            : 'bg-blue-500/10'

                        return (
                            <div key={`event:${block.id}`} className="py-2 px-1">
                                <div className={`
                                    rounded-lg border-l-4 ${borderColor} ${bgColor}
                                    p-3 shadow-sm
                                `}>
                                    <div className="flex items-start gap-2">
                                        <span className="text-lg">{presentation.icon}</span>
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium text-sm text-[var(--app-fg)]">
                                                {presentation.text}
                                            </div>
                                            {detail && (
                                                <div className="mt-1 text-xs text-[var(--app-hint)] line-clamp-3">
                                                    {detail}
                                                </div>
                                            )}
                                            <div className="mt-2 flex items-center gap-2 text-[10px] text-[var(--app-hint)]">
                                                ...
                                                <span className={`px-1.5 py-0.5 rounded ${
                                                    ...
                                                    : severity === 'medium' ? 'bg-purple-500/20 text-purple-400'
                    ...
                    // Special styling for Advisor status updates
                    if (isAdvisorStatus) {
                        return (
                            <div key={`event:${block.id}`} className="py-1">
                                <div className="mx-auto w-fit max-w-[92%] px-3 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20">
