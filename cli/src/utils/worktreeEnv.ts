...
function readWorktreeFromEnv(): WorktreeInfo | null {
    const basePath = process.env.YR_WORKTREE_BASE_PATH?.trim();
    const branch = process.env.YR_WORKTREE_BRANCH?.trim();
    const name = process.env.YR_WORKTREE_NAME?.trim();
    const worktreePath = process.env.YR_WORKTREE_PATH?.trim();
    const createdAtRaw = process.env.YR_WORKTREE_CREATED_AT?.trim();
